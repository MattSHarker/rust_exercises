...
fn main() {
    ...
    // first, create an enum with multiple types
    #[derive(Debug)]
    ...
    // then create a vector that holds the enum
    let row = vec![
        SpreadsheetCell::Int(3),
        SpreadsheetCell::Float(3.141),
        SpreadsheetCell::Text(String::from("x^2-3y")),
    ];
    // each instance of the enum can hold different types, but the vector
        // only cares that they're all the same enum type
